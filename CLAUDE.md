...
13. **Deprecated kod / yorum YASAK.** Yeni algoritma yazıldığında eski evaluator/snapshot/test/seed kalıntıları SİLİNİR. Yorumlu kod blokları (`/* ... */`, `// removed`), eski commented-out param JSON'ları, eski Type enum kalıntıları kalmaz. Sadece aktif kullanılan kod. Geçmiş loop raporlarında zaten dokümante.
